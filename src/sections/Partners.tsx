  ...
  return (
    <div>
      ...
      <ul className="mt-8 grid grid-cols-2 items-center gap-x-8 gap-y-8 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        {partners.map((item) => (
          <li key={item.src} className="flex h-16 items-center justify-center">
            <img src={item.src} alt={item.name} className="max-h-12 max-w-full object-contain grayscale opacity-80" />
          </li>
        ))}
      </ul>
    </div>
